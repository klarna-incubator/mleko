...
class TestPipelineStep:
    ...
    class DummyPipelineStep(PipelineStep):
        ...
        def execute(self, data_container: DataContainer):
            ...
            data_container.data[self._outputs[0]] = "0"  # type: ignore
            data_container.data[self._outputs[1]] = "1"  # type: ignore
            data_container.data[self._outputs[2]] = "2"  # type: ignore
            return data_container

    def test_init(self):
        """Should init a concrete PipelineStep subclass."""
        concrete_step = self.DummyPipelineStep(inputs=["input"], outputs=["first", "second", "third"], cache_group=None)
    ...
    def test_wrong_number_inputs_outputs(self):
        ...
        with pytest.raises(ValueError):
            self.DummyPipelineStep(inputs=["input1", "input2"], outputs=["first", "second", "third"], cache_group=None)

        with pytest.raises(ValueError):
            self.DummyPipelineStep(inputs=["input"], outputs=["first", "second"], cache_group=None)
