...
class TestVarianceFeatureSelector:
    ...
    def test_default_invariant(self, temporary_directory: Path, example_vaex_dataframe: vaex.DataFrame):
        ...
        df = invariance_feature_selector._select_features(example_vaex_dataframe)
        assert df.shape == (5, 3)
    ...
    def test_cache(self, temporary_directory: Path, example_vaex_dataframe: vaex.DataFrame):
        ...
        df = InvarianceFeatureSelector(temporary_directory).select_features(example_vaex_dataframe)
        assert df.shape == (5, 3)
        assert df.column_names == ["a", "d", "e"]

        with patch.object(InvarianceFeatureSelector, "_select_features") as mocked_select_features:
            InvarianceFeatureSelector(temporary_directory).select_features(example_vaex_dataframe)
