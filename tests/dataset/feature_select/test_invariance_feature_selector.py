...
class TestInvarianceFeatureSelector:
    ...
    def test_cache(self, temporary_directory: Path, example_vaex_dataframe: vaex.DataFrame):
        ...
        _, df = InvarianceFeatureSelector(temporary_directory).fit_transform(example_vaex_dataframe)
        assert df.shape == (5, 3)
        assert df.column_names == ["a", "d", "e"]

        with patch.object(InvarianceFeatureSelector, "_fit_transform") as mocked_fit_transform:
            InvarianceFeatureSelector(temporary_directory).fit_transform(example_vaex_dataframe)
