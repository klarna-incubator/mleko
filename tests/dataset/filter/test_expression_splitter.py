...
class TestExpressionFilter:
    ...
    def test_split_by_index(self, temporary_directory: Path, example_vaex_dataframe: vaex.DataFrame):
        ...
        df = test_expression_filter._filter(example_vaex_dataframe)
        assert df.shape == (4, 4)
    ...
    def test_split_cache(self, temporary_directory: Path, example_vaex_dataframe: vaex.DataFrame):
        ...
        test_expression_filter.filter(example_vaex_dataframe)

        with patch.object(ExpressionFilter, "_filter") as mocked_split:
            test_expression_filter.filter(example_vaex_dataframe)
