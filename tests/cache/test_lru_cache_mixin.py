...
class TestLRUCacheMixin:
    ...
    class MyTestClass(LRUCacheMixin):
        ...
        def __init__(self, cache_directory, max_entries):
            """Initialize cache."""
            super().__init__(cache_directory, max_entries)
    ...
    def test_clean_cache_on_load(self, temporary_directory: Path):
        ...
        for i in range(n_cache_entries + 3):
            new_file = temporary_directory / f"MyTestClass.test.{cache_file_prefix_name}{i}.pkl"
            new_file.touch()
            new_file_extra = temporary_directory / f"MyTestClass.test.{cache_file_prefix_name}{i}.extra"
        ...
        cache_file_keys = list(temporary_directory.glob(f"*{cache_file_prefix_name}*.*"))
    ...
    def test_two_classes_same_cache(self, temporary_directory: Path):
        ...
        lru_cached_class = self.MyTestClass(temporary_directory, 2)
        lru_cached_class2 = self.MyTestClass2(temporary_directory, 2)
    ...
    def test_disabled_cache(self, temporary_directory: Path):
        ...
        my_test_instance = self.MyTestClass(temporary_directory, 0)
